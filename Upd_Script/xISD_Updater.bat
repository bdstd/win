@echo off
cls
title iShareDisk Server 20230414 Updater v1.0.5 - By bdstd@2026
...
cls
echo Updating iShareDisk Server...
...
REM ========================== iShareDisk Update Section
:isharedisk_kill_process
taskkill /f /im iShareAdmin.exe >nul 2>&1
taskkill /f /im iSharedisk.exe >nul 2>&1
taskkill /f /im iShareDiskHelperLauncher.exe >nul 2>&1
taskkill /f /im iShareDiskHelper.exe >nul 2>&1

:isharedisk_update_files
xcopy /s /y /f _Temp_Update_\*.* >nul 2>&1
rd /s /q _Temp_Update_ >nul 2>&1
rd /s /q _Temp_Custom_ >nul 2>&1
rd /s /q _Temp_Fix_ >nul 2>&1

:isharedisk_begin
sc query iShareDiskService | find /i "iShareDiskService" >nul || ( start "" _install_or_repair.bat & goto update_done )

:register_firewall
echo [+] Registering Firewall
netsh advfirewall firewall delete rule name="iShareDisk" >nul 2>&1
netsh advfirewall firewall add rule name="iShareDisk" program="%cd%\iShareDisk.exe" dir=in profile=any action=allow >nul 2>&1
netsh advfirewall firewall delete rule name="iShareAdmin" >nul 2>&1
netsh advfirewall firewall add rule name="iShareAdmin" program="%cd%\iShareAdmin.exe" dir=in profile=any action=allow >nul 2>&1
...
:start_isharedisk
net start iShareDiskService >nul 2>&1
start "" iShareAdmin.exe --boot
start /min iShareDiskHelper\_register_service.bat
start /min iShareDiskExtFLauncher_Register.bat
REM ========================== End Of iShareDisk Update Section
